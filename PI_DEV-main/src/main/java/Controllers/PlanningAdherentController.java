...
public class PlanningAdherentController implements Initializable {
    ...
    private void ParticiperSeance(Seance seance) {
        String path;
        // Vérifier le type de séance et choisir le FXML correspondant
        if (seance.getType() == Type.ENREGISTRE) {
            path = "/regarderVideo.fxml";
        } else {
            path = "/livestreamAdherent.fxml";
        }
        ...
        try {
            ...
            // Transmettre les informations de la séance au contrôleur de la vue chargée
            if (seance.getType() == Type.ENREGISTRE) {
                // Pour regarderVideo.fxml, on suppose que le contrôleur a une méthode initData(Seance)
                Controllers.RegarderVideo controller = loader.getController();
                controller.initData(seance);
            } else {
                // Pour livestreamAdherent.fxml, on suppose qu'il existe un contrôleur LivestreamAdherentController
                //Controllers.LiveStreamController controller = loader.getController();
               // controller.initData(seance);
            }

            // Récupérer la scène à partir d'un noeud (ici gridSeance ou tout autre noeud de la vue)
            Stage stage = (Stage) gridSeance.getScene().getWindow();
